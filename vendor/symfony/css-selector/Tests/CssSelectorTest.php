...
class CssSelectorTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testParseExceptions()
    {
        try {
            ...
            $this->fail('->parse() throws an Exception if the vendor selector is not valid');
        } catch (\Exception $e) {
            $this->assertInstanceOf('\Symfony\Component\CssSelector\Exception\ParseException', $e, '->parse() throws an Exception if the vendor selector is not valid');
            $this->assertEquals('Expected identifier, but <eof at 3> found.', $e->getMessage(), '->parse() throws an Exception if the vendor selector is not valid');
        }
    }
